...
const FormData = ({ user }) => {
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading("SEND");
    if (user) {
      const content = await addComment(textComment);
    ...
    <div className="my-8 w-full max-w-xl">
      <div className="mb-2 flex select-none items-end gap-2">
        ...
        <p className="text-sm text-neutral-500">
          Type as {">"} {user.user_metadata.name}
        </p>
      </div>
      <textarea
        name="textComment"
...
export default dynamic(() => Promise.resolve(FormData), {
  loading: () => (
    <div className="my-[4.4rem] h-[159px] w-full animate-pulse rounded-md bg-neutral-900 sm:w-[36rem]"></div>
  ),
  ssr: false,
});
