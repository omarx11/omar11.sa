...
export default function Projects({ repo, reposInfo }) {
  const [hovoredCard, setHovoredCard] = useState(false);

  return (
    <div
      onMouseEnter={() => setHovoredCard(true)}
      onMouseLeave={() => setHovoredCard(false)}
          ...
          <Tooltip open={hovoredCard}>
            ...
            <TooltipContent
              ...
              className={cn("select-none", {
                "bg-emerald-800": reposInfo[repo.name]?.isUnderDev === false,
                "bg-orange-800": reposInfo[repo.name]?.isUnderDev === true,
              })}
            >
              {reposInfo[repo.name].isUnderDev !== true
                ? "Complete"
